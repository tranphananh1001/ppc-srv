...
gulp.task('imagesapp2', function() {
  return gulp.src('./bower_components/datatables/media/css/DataTables-1.10.11/images/**')
    .pipe(gulp.dest('./dist/css/DataTables-1.10.11/images'))
  ;
});
gulp.task('countryapp', function() {
  return gulp.src('./bower_components/country-select-js/**')
    .pipe(gulp.dest('./dist/country-select-js'))
  ;
});
/**
 * Dist
 */
gulp.task('dist', ['vendors', 'assets', 'fonts','fontsboot', 'fontsapp', 'imagesapp','imagesapp2','countryapp', 'styles-dist', 'scripts-dist'], function() {
  return gulp.src('./src/app/index.html')
    .pipe(g.inject(gulp.src('./dist/css/vendors.min.{js,css}'), {
      ignorePath: 'dist',
      starttag: '<!-- inject:vendor:{{ext}} -->'
    }))
    .pipe(g.inject(gulp.src('./dist/css/' + bower.name + '.min.{js,css}'), {ignorePath: 'dist'}))
    .pipe(replace({
      patterns: [
        {
          match: 'backendUrl',
...
gulp.task('statics', g.serve({
  hostname: settings.frontend.hostname.development,
...
gulp.task('production', g.serve({
  hostname: settings.frontend.hostname.production,
  port: settings.frontend.ports.production,
  https: {
    key: settings.frontend.https.production.key,
    cert: settings.frontend.https.production.cert
  },
  root: ['./dist'],
