(function () {
  ...
  angular.module('frontend.ppc.product')
    .controller('productAbSplitTestingController', [
        '$scope', '$rootScope', '$state', '$stateParams',
        'UserService', 'MessageService', '$http', 'HomeModel',
        'ProductModel', 'SettingsModel', 'CampaignModel', 'CampaignPerfomanceReportModel', 'DTOptionsBuilder', '$timeout', 'DataService',
        ...
        $scope.init = function() {
          ...
          $scope.$watch('itemSelected', $scope.onCategoryChange, true);
      	};

        $scope.initPageData = function() {
            $scope.PAGE_ADD_SHOW = false;
        ...
        $scope.reload_test = function () {
            ProductModel
                .getalltests({user: $scope.user.id, id: $scope.$parent.productId})
                .then(function (response) {
                ...
                ProductModel.savenewtest(item)
                    .then(function (response) {
                        MessageService.success('Test saved');
                            ...
                            $scope.reload_test();
                            $scope.test_show(response.insertId);
                    });
            } else {
        ...
        $scope.uploadFile = function (a, files) {
            ...
            $http.post(DataService.getBackendConfig().url + '/api/uploadphoto', fd, {
